...
public class SwapFileTest
{
    ...
    public static void testPages() {
        ...
        Vector<int []> pageVector = new Vector<int[]>();
        int[] pages = kernel.malloc(1);
        // Allocate all the memory that we can
        while (null == pages || 0 == pages.length)
        {
        	pageVector.add(pages);
            pages = kernel.malloc(1);
        }
        int ppn1 = 0;
        // Throw something into the memory
        for (int i = 0; i < pageVector.size(); i++)
        {
            ppn1 = pageVector.get(i)[0];
            myPages.add(ppn1); // so we can free the later
            text.loadPage(0, ppn1);
        }
        // try to load one more, which hsould trigger a swap
        pages = kernel.malloc(1);
        if (null == pages || 0 == pages.length) {
            System.err.println(
                    "Unable to malloc enough space to run selfTest");
        ...
        int ppn2 = pages[0];
        myPages.add(ppn2);
        text.loadPage(0, ppn2);
        // Since we loaded the same page in all the memory slots, we should be fine testing this
