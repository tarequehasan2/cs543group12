...
//			socket.setSocketState(CLOSED);
	...
	private static void doStpSent(SocketOpenFile socket, SocketEvent event) throws ProtocolError {
		switch (event){
		case TIMER:
...
//			socket.setSocketState(CLOSED);
	...
	private static void doStpRcvd(SocketOpenFile socket, SocketEvent event) throws ProtocolError, FailSyscall {
		switch (event){
		case RECV:
//			socket.dequeue();
			break;
...
//			socket.setSocketState(CLOSED);
	...
	private static void doEstablished(SocketOpenFile socket, SocketEvent event) throws ProtocolError {
		switch (event){
		case RECV:
//			socket.dequeue();
			break;
...
//			socket.setSocketState(CLOSED);
	...
	private static void doSynRcvd(SocketOpenFile socket, SocketEvent event) throws ProtocolError {
		switch (event){
		case ACCEPT:
//			socket.send(SYNACK);
//			socket.setSocketState(ESTABLISHED);
	...
	private static void doSynSent(SocketOpenFile socket, SocketEvent event) throws ProtocolDeadlock, ProtocolError {
		switch (event) {
		case TIMER:
		case DATA:
		case STP:
